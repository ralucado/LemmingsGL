...
bool Text::init(const char *filename)
{
	...
	quad = MaskedTexturedQuad::createTexturedQuad(geom, texCoords, simpleTexProgram);
...
void Text::render(char c, const glm::vec2 &pixel, int size, const glm::vec4 &color)
{
	...
	simpleTexProgram.setUniform4f("color", color.r, color.g, color.b, color.a);
	...
	simpleTexProgram.setUniform2f("minTexCoord", minTexCoord.s, minTexCoord.t);
	simpleTexProgram.setUniform2f("maxTexCoord", maxTexCoord.s, maxTexCoord.t);
